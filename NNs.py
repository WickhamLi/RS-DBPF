...
class dynamic_RSNN(nn.Module): 
    def __init__(self, input, hidden, output): 
        ...
        m = 8
        self.input = input
        self.fc_list = [nn.Sequential(nn.Linear((input), (hidden), dtype=torch.float32), 
                                 nn.Tanh(), 
                                 nn.Linear((hidden), (output), dtype=torch.float32))
                                 for i in range(m)]

    def forward(self, s, m): 
        out = self.fc_list[m](s.view(-1, self.input))
        return torch.squeeze(out)
